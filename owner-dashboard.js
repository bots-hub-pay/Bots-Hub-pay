document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.owner-dashboard-page').style.display = 'block';
    loadDashboardData();
...
function renderApprovals(approvals) {
    ...
    container.innerHTML = pending.map(approval => {
        ...
        switch(approval.type) {
            case 'add_fund':
                typeText = '💰 Add Fund'; icon = 'fa-solid fa-circle-plus';
                details = `Amount: <span>₹${amount.toFixed(2)}</span> | Method: ${approval.method || 'UPI'}`;
                if (approval.utr) details += ` | UTR: ${approval.utr}`;
                break;
            case 'withdraw':
                typeText = '🏦 Withdraw'; icon = 'fa-solid fa-arrow-right';
                details = `Amount: <span>₹${amount.toFixed(2)}</span> | Method: ${approval.method} | Account: ${approval.account}`;
                break;
            case 'pay_user':
                typeText = '💸 Pay to User'; icon = 'fa-solid fa-paper-plane';
                details = `Amount: <span>₹${amount.toFixed(2)}</span> | To: ${approval.recipientName} (${approval.recipientPhone})${approval.note ? ' | Note: ' + approval.note : ''}`;
                break;
        }
        return `<div class="approval-item" id="approval-${approval.id}">
            <div class="approval-info">
                <div class="type"><i class="${icon}"></i> ${typeText} <span class="pending-badge">Pending</span></div>
                <div class="details">${details} | From: ${approval.userName} (${approval.userPhone}) | ${timeStr}</div>
            </div>
            <div class="approval-actions">
                <button class="approve-btn" onclick="approveRequest('${approval.id}')"><i class="fa-solid fa-check"></i> Approve</button>
                <button class="reject-btn" onclick="rejectRequest('${approval.id}')"><i class="fa-solid fa-xmark"></i> Reject</button>
            </div>
        </div>`;
    }).join('');
}
...
function processTransaction(approval) {
    ...
    switch(approval.type) {
        case 'add_fund':
            ...
            user.activities.unshift({ text: `✅ Added ₹${approval.amount.toFixed(2)} (Approved)`, icon: 'fa-solid fa-circle-plus', color: 'green', time: new Date().toISOString() });
            if (!user.transactions) user.transactions = [];
            user.transactions.unshift({ type: 'add', amount: approval.amount, description: 'Added funds (Approved)', time: new Date().toISOString() });
            break;
        case 'withdraw':
            user.balance = (user.balance || 0) - approval.amount;
            user.totalWithdrawn = (user.totalWithdrawn || 0) + approval.amount;
            if (!user.activities) user.activities = [];
            user.activities.unshift({ text: `✅ Withdrew ₹${approval.amount.toFixed(2)} (Approved)`, icon: 'fa-solid fa-arrow-right', color: 'orange', time: new Date().toISOString() });
            if (!user.transactions) user.transactions = [];
            user.transactions.unshift({ type: 'withdraw', amount: approval.amount, description: `Withdrew to ${approval.account} (Approved)`, time: new Date().toISOString() });
            break;
        case 'pay_user':
            ...
            if (!user.activities) user.activities = [];
            user.activities.unshift({ text: `✅ Sent ₹${approval.amount.toFixed(2)} to ${recipient.fullName || approval.recipientPhone} (Approved)`, icon: 'fa-solid fa-paper-plane', color: 'purple', time: new Date().toISOString() });
            if (!user.transactions) user.transactions = [];
            user.transactions.unshift({ type: 'sent', amount: approval.amount, description: `Sent to ${recipient.fullName || approval.recipientPhone}`, time: new Date().toISOString() });
            ...
            recipient.activities.unshift({ text: `✅ Received ₹${approval.amount.toFixed(2)} from ${user.fullName || approval.userPhone} (Approved)`, icon: 'fa-solid fa-arrow-down', color: 'green', time: new Date().toISOString() });
            if (!recipient.transactions) recipient.transactions = [];
            recipient.transactions.unshift({ type: 'received', amount: approval.amount, description: `Received from ${user.fullName || approval.userPhone}`, time: new Date().toISOString() });
